...
rec {
    ...
    let
      matcher =
        ...
        let
          ...
          armBased = builtins.any (s: hasInfix s cpu) [
            "arm"
            "amd"
            "aarch"
          ];
          archMatch = (isAarch && armBased) || !(isAarch || armBased);
          bitMatch = (is64bit && (hasSuffix "64" cpu)) || (is32bit && (hasSuffix "86" cpu));
        ...
        in
        (universal || (archMatch && bitMatch)) && osMatch;

      targets = builtins.filter matcher (attrs.targets or [ ]);
    in
    (builtins.removeAttrs attrs [ "targets" ]) // { inherit targets; };
  ...
  filterGemset =
    ...
    in
    mapAttrs targetMatches converged;
}
